    ...
    <a-form
      ...
      <a-form-item :label="$t('compute.text_177')" class="mb-0" v-bind="formItemLayout" v-if="!isInstallOperationSystem">
        <cloudregion-zone
          ...
          :cloudregion-params="regionParams"
...
export default {
  name: 'BaremetalPrivateCreate',
  mixins: [CreateMixin],
  computed: {
    decorators () {
      const imageTypeInitValue = IMAGES_TYPE_MAP.private.key
      return {
        ...this.commonDecorators,
        imageOS: {
          prefer_manager: [
            'prefer_manager',
            {
    ...
    regionParams () {
      const regionParams = {
        ...this.params.region,
        provider: PROVIDER_MAP.Cloudpods.key,
      }
      return regionParams
    },
    vpcParams () {
      const vpcParams = {
        ...this.params.vpcParams,
        filter: 'external_id.equals("default")',
      }
      return vpcParams
    },
    cacheImageParams () {
      ...
      params.cloudregion_id = this.form.fd.cloudregion?.id
